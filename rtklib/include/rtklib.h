...
#define PATCH_LEVEL "b34c"               /* patch level */
...
#define RMODE_IGG1 1
#define RMODE_IGG3 2
#define RMODE_HUBER 3
#define RMODE_CAUCHY 4
...
typedef struct {        /* solution type */
    ...
    double rel_vel[3];
    double windows_vel[10][3];
    double predict_vel[3];//拟合速度
    double para[12];
    int windows_size;
    int init;
    double vv[3]; //用于速度约束
...
typedef struct {        /* processing options type */
    ...
    double maxinno[3];     /* reject threshold of innovation (m) */
	...
	int robustopt;      /* robust type (0:none,1:IGG1,2:IGG3,3:Huber,4:Cauchy) */
	double k0;
	double k1;
    ...
    int StateModel=1;
    double Dop2PrRatio=10.0;
    int DopplerConstraint=0;//在差分模式下使用多普勒进行速度约束
    double thres[2]{100,2.5};
    bool DataCheck = true;
    int usetruth = 0;
    double default_rb[3];
    double default_snr_max;
    double thresdop;
...
typedef struct {        /* satellite status type */
    ...
    double resd[NFREQ]; /* residuals of doppler (m/s) */
    ...
    double ref_e[3];
    double ref_dts;
    double ref_dion[NFREQ];
    double ref_dtrp[NFREQ];
    double restdcp[NFREQ];

    int outlier_obs[3][NFREQ];
    ...
    double P_COR[NFREQ+NEXOBS];
    double tgd[NFREQ+NEXOBS];
} ssat_t;
...
EXPORT int  lsq   (const double *A, const double *y, int n, int m, double *x,
                   double *Q);
EXPORT int lsq_R (const double *A, const double *y, const double * R, int n, int m, double *x,
                double *Q);
EXPORT int lsq_rtk (const double *A, const double *y, const double * R, int n, int m, const double *x,double *xx,
...
EXPORT double Wi(int fname, double v, double k0, double k1);
EXPORT void robust_lsq(const double *H0,const double* v0,const double* P,int n,int m,int p,double* H,double* v,const prcopt_t *opt,int iter);
EXPORT double robust(double *x,double *H,double* v,double *P,double* R,int n,int m,double* RR,const prcopt_t *opt,int *type);
...
EXPORT void matprintT(const char mark[],const double A[], int n, int m, int p, int q);
...
public:
    ...
    /* GLO-GPS time offset (s) */
    /* GAL-GPS time offset (s) */
    /* BDS-GPS time offset (s) */
    /* IRN-GPS time offset (s) */
...
extern std::queue<GNSS_Epoch> gnss_buf;
...
extern FILE* fp_dop;
#endif
extern double para_Vel[1][3];
extern std::map<long, Eigen::Vector3d> BasePos;
