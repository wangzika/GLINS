...
std::map<long, Eigen::Vector3d> BasePos;
    ...
    sta_t* sta)
{
    ...
    while (fgets(buff, MAXRNXLEN, fp))
    {
        ...
        ///TODO:基站位置变化
        {
            if (strstr(buff, "EVENT"))
            {
                ...
                if (strstr(buff, "APPROX POSITION XYZ"))
                {
                    ...
                    sscanf(str, "%lf %lf %lf", pos, pos + 1, pos + 2);
        ...
        if (i == 0)
        {
            ...
            if ((double)time.time == 0.0) continue;
            //读观测值时间的时候，判断newbase
            if (newbase)
            {
                newbase = false;
                long timestamp = time.time;
                Eigen::Vector3d base(pos[0], pos[1], pos[2]);
                BasePos[timestamp] = base;
            }
            if (*flag == 5)
            {
