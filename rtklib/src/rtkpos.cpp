...
static void outsolstat(rtk_t* rtk, const nav_t* nav)
{
    ...
    for (i = 0; i < MAXSAT; i++)
    {
        ...
        for (j = 0; j < nfreq; j++)
        {
            ...
            //            fprintf(fp_stat,"$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.0f,%d,%d,%d,%d,%d,%d,%.2f,%.6f,%.5f\n",
            //                    week,tow,id,j+1,ssat->azel[0]*R2D,ssat->azel[1]*R2D,
            //                    ssat->restdcp[j],ssat->resc[j],ssat->vsat[j],ssat->snr_rover[j]*SNR_UNIT,
            //                    ssat->fix[j],ssat->slip[j]&3,ssat->lock[j],ssat->outc[j],
            //                    ssat->slipc[j],ssat->rejc[j],rtk->x[k],
            //                    rtk->P[k+k*rtk->nx],ssat->icbias[j]);
        }
    ...
    int ObsType)
{
    ...
    if (ObsType == 1)
        ...
        fact = opt->eratio[frq] / opt->Dop2PrRatio;
...
{
    double* F, * P, * FP, * x = nullptr, * xp, pos[3], Q[9] = { 0 }, Qv[9], var = 0.0;
    int i, j, * ix, nx;
    ...
    // 使用拟合速度
    if (rtk->opt.StateModel == 3)
    {
        Eigen::Matrix<double, 4, 3> para;
        matcpy(para.data(), rtk->sol.para, 12, 1);
        Eigen::Matrix<double, 1, 4> H;
        H << 1, 11, 11 * 11, 11 * 11 * 11;
        Eigen::Vector3d predict_vel = H * para;
        matcpy(rtk->sol.predict_vel, predict_vel.data(), 3, 1);
        double rb_pos[3], dx[3];
        ecef2pos(rtk->rb, rb_pos);
        enu2ecef(rb_pos, rtk->sol.predict_vel, dx);
        for (i = 0; i < nx; i++)
            x[i] = rtk->x[ix[i]];
        for (i = 0; i < 3; i++)
            x[i] = dx[i] + rtk->rb[i];
        ...
        if (Qdv[0] != 100.0)
        {
            for (i = 0; i < 3; i++)
                for (j = 0; j < 3; j++)
                {
                    rtk->P[i + j * rtk->nx] += Qdv[i + j * 3];
                }
        }
    }

    if (rtk->opt.StateModel == 2)
    {
        double rb_pos[3], vel_enu[3];
        ecef2pos(rtk->rb, rb_pos);
        ecef2enu(rb_pos, rtk->sol.vv, vel_enu);
        //        if (abs(vel_enu[0])<0.05 && abs(vel_enu[1])<0.05 && abs(vel_enu[2])<0.05){
        //            return;
        //        }
        //        if (abs(vel_enu[2])>1.0){
        //            /* reset position with large variance */
        ...
        for (i = 0; i < nx; i++)
            x[i] = rtk->x[ix[i]];
        for (i = 0; i < 3; i++)
            x[i] += rtk->opt.usetruth ? rtk->sol.rel_vel[i] * tt : rtk->sol.vv[i] * tt;
        //            x[i]+=rtk->sol.rel_vel[i];
        //            x[i]+=rtk->sol.rr[3+i]*tt;
        for (i = 0; i < nx; i++)
            rtk->x[ix[i]] = x[i];
        ...
        {
            if (rtk->sol.qvv[0] != 0.0 && rtk->sol.qvv[4] != 0.0 && rtk->sol.qvv[8] != 0.0)
            {
                //                Qdv[0] = rtk->sol.qvv[0];
                //                Qdv[4] = rtk->sol.qvv[4];
                //                Qdv[8] = rtk->sol.qvv[8];
                memcpy(Qdv, rtk->sol.qvv, sizeof(double) * 9);
            }
            else
            {
                Qdv[0] = 0.001;
        ...
        //        Qdv[0]=rtk->sol.checkqv[0]*tt;
        //        Qdv[4]=rtk->sol.checkqv[1]*tt;
        //        Qdv[8]=rtk->sol.checkqv[2]*tt;
        //        Qdv[1]=Qdv[3]=rtk->sol.checkqv[3]*tt;
        //        Qdv[5]=Qdv[7]=rtk->sol.checkqv[4]*tt;
        //        Qdv[2]=Qdv[6]=rtk->sol.checkqv[5]*tt;
        //        Qdv[0]=0.0001;
        //        Qdv[4]=0.0001;
        //        Qdv[8]=0.0001;
        //        Qdv[0]=rtk->sol.qv[0];
        //        Qdv[4]=rtk->sol.qv[1];
        //        Qdv[8]=rtk->sol.qv[2];
        //        Qdv[1]=Qdv[3]=rtk->sol.qv[3];
        //        Qdv[5]=Qdv[7]=rtk->sol.qv[4];
        //        Qdv[2]=Qdv[6]=rtk->sol.qv[5];

        if (Qdv[0] != 100.0)
        {
            //            for(i=0;i<9;i++) Qdv[i]*=1e6;
            //            matprintT("Q",Qdv,3,3,10,7);
            for (i = 0; i < 3; i++)
                for (j = 0; j < 3; j++)
                {
                    rtk->P[i + j * rtk->nx] += Qdv[i + j * 3] * tt;
                }
        }
    }
    ...
    double vv = SQR(rtk->x[0] - rtk->sol.rr[0]) + SQR(rtk->x[1] - rtk->sol.rr[1]) + SQR(rtk->x[2] - rtk->sol.rr[2]);
    double v0 = sqrt(vv / (rtk->P[0 + 0 * rtk->nx] + rtk->P[1 + 1 * rtk->nx] + rtk->P[2 + 2 * rtk->nx]));
    //    printf("\nv0 %.3lf\n",v0);
    if (rtk->opt.StateModel == 1)
    {
        double rb_pos[3], vel_enu[3];
        ecef2pos(rtk->rb, rb_pos);
        ecef2enu(rb_pos, rtk->x + 3, vel_enu);
        double vel_u;
        //        if (abs(vel_u = vel_enu[2])>1.0){
        //            vel_enu[2] = 0.001;
        //            enu2ecef(rb_pos,vel_enu,rtk->x+3);
        //            /* reset position with large variance */
        ////            for (i=0;i<3;i++) initx(rtk,rtk->x[i],100,i);
        ////            return;
        //        }
        for (i = 0; i < 6; i++)
        {
        ...
        if (var < rtk->opt.thresar[1] && fabs(rtk->x[6]) < 10 && fabs(rtk->x[7]) < 10 && fabs(rtk->x[8]) < 10)
        {
            for (i = 0; i < 3; i++)
            {
        ...
        for (i = 0; i < nx; i++)
        {
            ...
            for (j = 0; j < nx; j++)
            {
                ...
                //                rtk->P[ix[i]+ix[j]*rtk->nx]=(abs(vel_u)>1.0?abs(vel_u):1.0)*P[i+j*nx];
            }
        }
...
static void zdres_sat(int base, double r, const obsd_t* obs, const nav_t* nav,
    ...
    double rate, double rcv_clk_drift, const double sat_clk_drift)
{
    ...
    else
    {
        for (i = 0; i < nf; i++)
        {
            ...
            if (opt->DopplerConstraint)
                if (obs->D[i] != 0.0 && base == 0)
                    y[i + 2 * nf] = -CLIGHT / freq[i] * obs->D[i] - (rate + rcv_clk_drift - CLIGHT * sat_clk_drift);
        }
    }
}
...
static int zdres(int base, const obsd_t* obs, int n, const double* rs,
    ...
    const nav_t* nav, const double* rr, prcopt_t* opt,
    int index, double* y, double* e, double* azel, double* freq)
{
    ...
    int ObsTN = opt->DopplerConstraint ? 3 : 2;

    trace(3, "zdres   : n=%d\n", n);

    double rate = 0;
    /* rv_ = local copy of rcvr vel */
    double rv_[4] = { 0 };
    for (i = 0; i < 3; i++)
    {
        rv_[i] = rr[3 + i];
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (opt->DopplerConstraint && norm(rs + 3 + i * 6, 3) <= 0.0)
            continue;
        ...
        if (opt->DopplerConstraint && base == 0)
        {
            ...
            for (j = 0; j < 3; j++)
                vs[j] = rs[j + 3 + i * 6] - rv_[j];
            /* range rate with earth rotation correction */
            rate = dot(vs, e_doppler, 3) + OMGE / CLIGHT * (rs[4 + i * 6] * rr[0] + rs[1 + i * 6] * rv_[0] - rs[3 + i * 6] * rr[1] - rs[i * 6] * rv_[1]);
        }

        for (f = 0; f < nf; f++)
        {
            if (obs[i].SNR[f] * SNR_UNIT > snr_max)
                snr_max = obs[i].SNR[f] * SNR_UNIT;
        }
        /* calc undifferenced phase/code residual for satellite */
        zdres_sat(base, r, obs + i, nav, azel + i * 2, dant, opt, y + i * nf * ObsTN, freq + i * nf, rate, 0, dts[i * 2 + 1]);
    }
    /// TODO:更新snr_max
    // if (base == 0) opt->err[5] = MIN(snr_max, opt->default_snr_max);
...
static int validobs(int i, int j, int f, int nf, double* y, int ObsTN)
{
    /* if no phase observable, psudorange is also unusable */
    /* 采用b34g的判断方式，即even no phase observable, pseudorange is still usable */
    if (f >= 2 * nf)
        return y[f + i * nf * ObsTN] != 0.0;

    return y[f + i * nf * ObsTN] != 0.0 && y[f + j * nf * ObsTN] != 0.0;
    //    &&(f<nf||(y[f-nf+i*nf*ObsTN]!=0.0&&y[f-nf+j*nf*ObsTN]!=0.0));
}
    ...
    int ns, double* v, double* H, double* R, int* vflg)
{
    ...
    Ri = mat(ns * nf * ObsTN + 2, 1);
    Rj = mat(ns * nf * ObsTN + 2, 1);
    im = mat(ns, 1);
    tropu = mat(ns, 1);
    tropr = mat(ns, 1);
    dtdxu = mat(ns, 3);
    ...
    for (m = 0; m < 6; m++)
    {
        ...
        for (f = opt->mode > PMODE_DGPS ? 0 : nf; f < nf * ObsTN; f++)
        {
            frq = f % nf;
            ...
            for (i = -1, j = 0; j < ns; j++)
            {
                ...
                if (!test_sys(sysi, m) || sysi == SYS_SBS)
                    continue;
                if (!validobs(iu[j], ir[j], f, nf, y, ObsTN))
                    continue;
                if (i >= 0 && rtk->ssat[sat[j] - 1].slip[frq] & LLI_SLIP)
                    continue;
                //                if (sat[j]==1&&frq==0) continue;
            ...
            for (j = 0; j < ns; j++)
            {
                ...
                if (!validobs(iu[j], ir[j], f, nf, y, ObsTN))
                    continue;
                //                if (sat[j]==1&&frq==0) continue;
                rtk->ssat[sat[i] - 1].lam[frq] = CLIGHT / freqi;
                rtk->ssat[sat[j] - 1].lam[frq] = CLIGHT / freqj;
                if (H)
                {
                    Hi = H + nv * rtk->nx;
                    for (k = 0; k < rtk->nx; k++)
                ...
                {
                    /* Doppler SD residual*/
                    v[nv] = y[f + iu[i] * nf * ObsTN] - y[f + iu[j] * nf * ObsTN];
                }
                else
                {
                    v[nv] = (y[f + iu[i] * nf * ObsTN] - y[f + ir[i] * nf * ObsTN]) -
                        (y[f + iu[j] * nf * ObsTN] - y[f + ir[j] * nf * ObsTN]);
                    //                    printf("v: %.3lf yiu: %.3lf yir: %.3lf yju: %.3lf yjr:%.3lf\n",
                    //                           v[nv],y[f + iu[i] * nf * ObsTN],y[f + ir[i] * nf * ObsTN],
                    //                           y[f + iu[j] * nf * ObsTN],y[f + ir[j] * nf * ObsTN]);
                }
                ...
                if (H)
                {
                    ...
                    for (k = 0; k < 3; k++)
                    {
                        Hi[k + offset] = -e[k + iu[i] * 3] + e[k + iu[j] * 3];
                    }
                }
                if (opt->ionoopt == IONOOPT_EST)
                {
                    /* adjust double-differenced measurements by double-differenced ionospheric delay term */
                    didxi = (ObsType == 1 ? -1.0 : 1.0) * im[i] * SQR(FREQL1 / freqi);
                    didxj = (ObsType == 1 ? -1.0 : 1.0) * im[j] * SQR(FREQL1 / freqj);
                ...
                threshadj = (P[ii + rtk->nx * ii] == SQR(rtk->opt.std[0])) ||
                    ...
                    ? 10
                    : 1;
                //                 printf("%d %d %lf %d\n",sat[j],frq,v[nv],ObsType);

                // threshadj = 1;
                if (fabs(v[nv]) > opt->maxinno[ObsType] * threshadj)
                {
                    rtk->ssat[sat[j] - 1].vsat[frq] = 0;
                    rtk->ssat[sat[j] - 1].rejc[frq]++;
                    if (ObsType == 0)
                    {
                        rtk->ssat[sat[j] - 1].vsatL[frq] = 0;
                    }
                    else if (ObsType == 1)
                    {
                        rtk->ssat[sat[j] - 1].vsatP[frq] = 0;
                    }
                    else if (ObsType == 2)
                    {
                        rtk->ssat[sat[j] - 1].vsatD[frq] = 0;
                    }
                    continue;
                }
                ...
                if (opt->mode > PMODE_DGPS)
                {
                    ...
                    if (ObsType == 0)
                    {
                        rtk->ssat[sat[i] - 1].vsatL[frq] = rtk->ssat[sat[j] - 1].vsatL[frq] = 1;
                    }
                    else if (ObsType == 1)
                    {
                        rtk->ssat[sat[i] - 1].vsatP[frq] = rtk->ssat[sat[j] - 1].vsatP[frq] = 1;
                    }
                    else if (ObsType == 2)
                    {
                        rtk->ssat[sat[i] - 1].vsatD[frq] = rtk->ssat[sat[j] - 1].vsatD[frq] = 1;
                    }
                }
                else
                {
                ...
                vflg[nv++] = (sat[i] << 16) | (sat[j] << 8) | (ObsType << 4) | (frq);
                nb[b]++;
            }
            b++;
        }
    ...
    if (rtk->opt.glomodear != GLO_ARMODE_FIXHOLD || rtk->holdamb)
    {
        ...
        if (rtk->opt.arfilter)
        {
            ...
            if (rerun)
            {
                ...
                printf("rerun AR with new sat removed\n");
    ...
    const nav_t* nav)
{
    ...
    /// TODO:基站位置变化
    // for (std::map<long, Eigen::Vector3d>::iterator it = BasePos.begin(); it != BasePos.end(); it++)
    // {
    //     if (it->first <= (long)obs[nu].time.time)
    //     {
    //         Eigen::Vector3d pos = it->second;
    //         matcpy(rtk->rb, pos.data(), 3, 1);
    //         matcpy(rtk->opt.rb, pos.data(), 3, 1);
    //         updateBase = true;
    //     }
    // }
    //    if(!updateBase) {
    //        matcpy(rtk->rb,rtk->opt.default_rb,3,1);
    //        matcpy(rtk->opt.rb,rtk->opt.default_rb,3,1);
    //    }
    //    if(BasePos.find((long)obs[nu].time.time)!=BasePos.end())
    //    {
    //        Eigen::Vector3d pos = BasePos[(long)obs[nu].time.time];
    //        matcpy(rtk->rb,pos.data(),3,1);
    //        matcpy(rtk->opt.rb,pos.data(),3,1);
    //    }
    ...
    /// 保存上一个历元的P矩阵
    double* P_pre, * x_pre;
    P_pre = mat(rtk->nx, rtk->nx);
    x_pre = mat(rtk->nx, 1);
    matcpy(P_pre, rtk->P, rtk->nx, rtk->nx);
    matcpy(x_pre, rtk->x, rtk->nx, 1);

    trace(3, "relpos  : nx=%d nu=%d nr=%d\n", rtk->nx, nu, nr);

    /* time diff between base and rover observations (usually zero) */
    dt = timediff(time, obs[nu].time);
    int ObsTN = rtk->opt.DopplerConstraint ? 3 : 2; // ObsType Count:Pseudorange Carrier-Phase Doppler
    ...
    for (i = 0; i < MAXSAT; i++)
    {
        ...
        for (j = 0; j < NFREQ; j++)
        {
            ...
            rtk->ssat[i].vsatL[j] = 0;
            rtk->ssat[i].vsatP[j] = 0;
            rtk->ssat[i].vsatD[j] = 0;
    ...
    {
        {
            ...
            for (i = 0; i < nu; i++)
            {
                if (!rtk->ssat[obs[i].sat - 1].vs)
                    continue;
                rtklib::GNSS_Info_ZD gnss_info;
                rtklib::satdt sat_info;
                for (j = 0; j < 3; j++)
                {
                    sat_info.pos[j] = rs[j + i * 6];
                    sat_info.vel[j] = rs[j + 3 + i * 6];
                }
                sat_info.clk_bias = dts[0 + i * 2];
                sat_info.clk_drift = dts[1 + i * 2];
                gnss_info.Sat = sat_info;
                gnss_info.Mea = obs2msg(obs[i]);
                gnss_info.ssat = ssat2msg(rtk->ssat[obs[i].sat - 1]);

                ZD_infos.push_back(gnss_info);
            }
            epoch.ZD_Infos.swap(ZD_infos);
            int week;
            epoch.weekSec = time2gpst(time, &week);
            epoch.gpsWeek = week;
            epoch.header.stamp.fromSec((double)round(time.time * 100.0 + time.sec * 100.0) / 100.0 - 18);
        }
    }
    ...
    if (!zdres(1, obs + nu, nr, rs + nu * 6, dts + nu * 2, var + nu, svh + nu, nav, rtk->rb, opt, 1,
        y + nu * nf * ObsTN, e + nu * 3, azel + nu * 2, freq + nu * nf))
    {
    ...
    if (opt->intpref)
    {
        dt = intpres(time, obs + nu, nr, nav, rtk, y + nu * nf * ObsTN);
    }
    ...
    //    if(opt->robustopt>RMODE_NONE)niter++;
    trace(3, "rover:  dt=%.3f\n", dt);
    ...
    for (i = 0; i < niter; i++)
    {
        ...
        double adap;
        if (opt->robustopt > RMODE_NONE)
        {
            adap = robust(xp, H, v, Pp, R, rtk->nx, nv, R, opt, type);
            //            printf("adap %.2lf\n",adap);
        }
        ...
        //        double dx[3];
        //        for (int k = 0; k < 3; ++k) {
        //            dx[k] = (xp[k] - rtk->sol.ref_rr[k])/rtk->tt;
        //        }
        //        double pos[3], enu[3];
        //        ecef2pos(rtk->rb, pos);
        //        ecef2enu(pos, dx, enu);
        //        if (rtk->sol.ref_rr[0]!=0.0&&rtk->sol.init) {
        //            if (abs(enu[2]) > 1.0) {
        //                matcpy(rtk->P, P_pre, n, 1);
        //                matcpy(rtk->x, x_pre, n, 1);
        //                free(rs);free(dts);free(var);free(y);free(e);free(azel);free(freq);
        //                free(xp);free(Pp);free(xa);free(v);free(H);free(R);free(bias);
        //                free(type);free(P_pre);free(x_pre);
        //                return 2;
        //            }
        //        }
        trace(4, "x(%d)=", i + 1);
    ...
    // todo:FGO preprocessor
    {
        for (i = 0; i < ns; i++)
        {
            ...
            rtk->ssat[sat[i] - 1].azel_b[0] = azel[ir[i] * 2];
            rtk->ssat[sat[i] - 1].azel_b[1] = azel[1 + ir[i] * 2];
            for (j = 0; j < nf; j++)
            {
                /* snr of base and rover receiver */
                rtk->ssat[sat[i] - 1].snr_rover[j] = obs[iu[i]].SNR[j];
        ...
        if (valpos(rtk, v, R, vflg, nv, 4.0))
        {
            ...
            for (i = 0; i < ns; i++)
                for (f = 0; f < nf; f++)
                {
                    ...
                    rtk->sol.ns++; /* valid satellite count by L1 */ // if (f==0)
                }
    ...
    //        for(i=0;i<ns;i++)
    //        {
    //            rtklib::GNSS_Info_SD gnss_info;
    //            obsd_t obsr=obs[iu[i]];
    //            obsd_t obsb=obs[ir[i]];
    //            bool vsat= false;
    //            for(j=0;j<nf;j++)
    //                if(rtk->ssat[obsr.sat-1].vsat[j]!=0) vsat= true;
    ////            if(!vsat) continue;
    ...
    //            for(j=0;j<3;j++)
    //            {
    //                satr.pos[j]=rs[j + iu[i] * 6];
    //                satb.pos[j]=rs[j + ir[i] * 6];
    //                satr.vel[j]=rs[j+3 + iu[i] * 6];
    //                satb.vel[j]=rs[j+3 + ir[i] * 6];
    //            }
    //            satr.clk_bias=dts[iu[i]*2];
    //            satb.clk_bias=dts[ir[i]*2];
    //            satr.clk_drift=dts[1+iu[i]*2];
    //            satb.clk_drift=dts[1+ir[i]*2];
    //            gnss_info.Sat_Rover=satr;
    //            gnss_info.Sat_Base=satb;
    //            gnss_info.ssat= ssat2msg(rtk->ssat[obs[iu[i]].sat-1]);
    //            SD_infos.push_back(gnss_info);
    //        }
    //        epoch.SD_Infos=SD_infos;
    //    }
    ...
    int nsat;
    double azeldop[MAXOBS * 2];
    for (i = nsat = 0; i < MAXSAT; i++)
    {
        if (!rtk->ssat[i].vs)
            continue;
        azeldop[2 * nsat] = rtk->ssat[i].azel[0];
        azeldop[2 * nsat + 1] = rtk->ssat[i].azel[1];
    }
    dops(nsat, azeldop, opt->elmin, rtk->sol.dop);
    ...
    { /* float solution */
        ...
        memcpy(rtk->sol.ref_rr, rtk->sol.rr, sizeof(double) * 6);
        rtk->nfix = 0;
    }
    for (i = 0; i < n; i++)
        for (j = 0; j < nf; j++)
...

    {
        ...
        if (stat != SOLQ_NONE)
        {
            ...
            covenu(pos, P, Q);
            epoch.amb.resize(NB(opt));
            memcpy(epoch.amb.data(), rtk->x + NR(opt), sizeof(double) * NB(opt));
            epoch.var[0] = Q[0];
            epoch.var[1] = Q[4];
            epoch.var[2] = Q[8];
            epoch.var[3] = rtk->sol.qr[3]; /* xy or en */
            epoch.var[4] = rtk->sol.qr[4]; /* yz or nu */
            epoch.var[5] = rtk->sol.qr[5]; /* zx or ue */
            epoch.velvar[0] = rtk->sol.qv[0];
            epoch.velvar[1] = rtk->sol.qv[1];
            epoch.velvar[2] = rtk->sol.qv[2];
            epoch.velvar[3] = rtk->sol.qv[3]; /* xy or en */
            epoch.velvar[4] = rtk->sol.qv[4]; /* yz or nu */
            epoch.velvar[5] = rtk->sol.qv[5]; /* zx or ue */

            //        epoch.var[1]=P[4];
            //        epoch.var[2]=P[8];

            nav_msgs::Odometry odometry;
            odometry.header.stamp.fromSec((double)time.time + time.sec - 18);
            odometry.pose.pose.position.x = rtk->sol.rr[0];
    ...
    if (stat != SOLQ_NONE)
        rtk->sol.stat = stat;
    //    rtk->opt.err[5] = rtk->opt.default_snr_max;
...
static void velfitting(rtk_t* rtk)
{
    Eigen::Matrix<double, 10, 3> v;
    Eigen::Matrix<double, 10, 4> H;
    Eigen::Matrix<double, 4, 3> x;

    for (int i = 0; i < rtk->sol.windows_size; ++i)
    {
        H(i, 0) = 1;
        H(i, 1) = (i + 1);
        H(i, 2) = (i + 1) * (i + 1);
        H(i, 3) = (i + 1) * (i + 1) * (i + 1);
        v(i, 0) = rtk->sol.windows_vel[i][0];
        v(i, 1) = rtk->sol.windows_vel[i][1];
        v(i, 2) = rtk->sol.windows_vel[i][2];
    }
    x = (H.transpose() * H).inverse() * H.transpose() * v;

    matcpy(rtk->sol.para, x.data(), 12, 1);
}
...
extern int rtkpos(rtk_t* rtk, const obsd_t* obs, int n, const nav_t* nav)
{
    ...
    rtk_t rtk_temp;
    memcpy(&rtk_temp, rtk, sizeof(rtk_t));
    /* relative potitioning */
    predict_state = relpos(rtk, obs, nu, nr, nav);
    outsolstat(rtk, nav);
