...
/* smooth */
extern void lgccsmooth(obs_t* obs, int ns, const prcopt_t* popt)
{
    double Ps[2][MAXSAT][NFREQ] = { { { 0 } } }, Lp[2][MAXSAT][NFREQ] = { { { 0 } } }, Pp[2][MAXSAT][NFREQ] = { { { 0 } } }, dcp, dpp, dps;
    int i, j, s, r, n[2][MAXSAT][NFREQ] = { { { 0 } } };
    obsd_t* p;
    //Lp Pp 表示前历元的伪距和相位，其中伪距是平滑后的伪距
    //Ps  表示平滑后的伪距
    trace(3, "csmooth: nobs=%d,ns=%d\n", obs->n, ns);
    //这是平滑所有伪距还是仅仅GPS？ 平滑了所有观测值，所有卫星和所有频率
    for (i = 0;i < obs->n;i++)
    {//i:0 —— obs->n,怎么是从0开始算呢？
//p存储了一颗卫星的观测值信息等； s表示卫星prn； r表示接收机num
        p = &obs->data[i]; s = p->sat; r = p->rcv;


        for (j = 0;j < NFREQ;j++)
        { //j:表示频率
            if (s <= 0 || MAXSAT < s) continue;//排除卫星号不对，测站号不对状况
            if (p->P[j] == 0.0 || p->L[j] == 0.0) continue;//排除无该频率的伪距或相位观测值的情况
            if (p->LLI[j]) n[r - 1][s - 1][j] = 0;//n[][][]：是平滑窗口吗？是的
            ...
            {
                double lam_carr, freq;
                freq = sat2freq(p->sat, p->code[j], &navs);
                lam_carr = CLIGHT / freq;
                dcp = lam_carr * (p->L[j] - Lp[r - 1][s - 1][j]);//当前历元与上历元载波差值，单位：m
                dpp = p->P[j] - Pp[r - 1][s - 1][j];//当前历元与上历元伪距(平滑后)差值，单位：m

                //相位平滑伪距：Hatch滤波
                Ps[r - 1][s - 1][j] = p->P[j] / n[r - 1][s - 1][j] + (Ps[r - 1][s - 1][j] + dcp) * (n[r - 1][s - 1][j] - 1) / n[r - 1][s - 1][j];
                dps = fabs(p->P[j] - Ps[r - 1][s - 1][j]);//观测伪距与平滑伪距差值，用于伪距粗差探测吗？

                ////Variable window
                if (popt->smoothwidth == 0)
                {//可变窗口
                    if (fabs(dcp - dpp) < 1 && dps < 1)
                    {//电离层残差和伪距粗差探测，阈值均为2m？这么小？？？

                        p->P[j] = Ps[r - 1][s - 1][j];//通过探测，改变原始伪距观测值为平滑后的观测值
                        ++n[r - 1][s - 1][j];//平滑窗口+1
                    }
                    else  n[r - 1][s - 1][j] = 0;//探测不通过，平滑窗口重置
                }

                //fixed window
                if (popt->smoothwidth > 0)
                {//固定窗口平滑，smoothwidth为固定窗口大小
                    ns = popt->smoothwidth;
                    if (++n[r - 1][s - 1][j] < ns) p->P[j] = Ps[r - 1][s - 1][j];//若未达到固定窗口，则
                    else n[r - 1][s - 1][j] = 0;//达到固定窗口值，重置？？？？
                }
            }
            Lp[r - 1][s - 1][j] = p->L[j];//记录 前历元的相位观测值
            Pp[r - 1][s - 1][j] = p->P[j];//记录 前历元的平滑后的伪距观测值
        }
    }
}

extern void lgcdsmooth(obs_t* obs, int ns, const prcopt_t* popt)
{
    double Ps[2][MAXSAT][NFREQ] = { { { 0 } } }, Dp[2][MAXSAT][NFREQ] = { { { 0 } } }, Pp[2][MAXSAT][NFREQ] = { { { 0 } } }, dcp, dpp, dps;
    int i, j, s, r, n[2][MAXSAT][NFREQ] = { { { 0 } } };
    obsd_t* p;

    gtime_t Tp[2][MAXSAT][NFREQ] = { {{0}} };
    int k = 0;
    trace(3, "csmooth: nobs=%d,ns=%d\n", obs->n, ns);
    ...
    for (i = 0;i < obs->n;i++)
    {
        ...
        if (p->P[j] == 0.0 || p->D[j] == 0.0) continue;
        k = i;
        break;
    }
    ...
    for (i = 0;i < obs->n;i++)
    {
        ...
        for (j = 0;j < NFREQ;j++)
        {
            ...
            //            if (timediff(p->time,Tp[r - 1][s - 1][j])>1.0){
            //                n[r - 1][s - 1][j] = 0;
            //            }
            ...
            else
            {
                ...
                freq = sat2freq(p->sat, p->code[j], &navs);
                lam_carr = CLIGHT / freq;
                dcp = -lam_carr * (p->D[j] + Dp[r - 1][s - 1][j]) / 2 * 1;//采样间隔t=1
                dpp = p->P[j] - Pp[r - 1][s - 1][j];
                Ps[r - 1][s - 1][j] = p->P[j] / n[r - 1][s - 1][j] + (Ps[r - 1][s - 1][j] + dcp) * (n[r - 1][s - 1][j] - 1) / n[r - 1][s - 1][j];
                dps = fabs(p->P[j] - Ps[r - 1][s - 1][j]);


                ////Variable window
                if (popt->smoothwidth == 0)
                {
                    if (fabs(dcp - dpp) < 2 && dps < 2)
                    {
                        p->P[j] = Ps[r - 1][s - 1][j];
                        ++n[r - 1][s - 1][j];
                    }
                ...
                {
                    ns = popt->smoothwidth;
                    if (++n[r - 1][s - 1][j] < ns) p->P[j] = Ps[r - 1][s - 1][j];
                    else n[r - 1][s - 1][j] = 0;
                }
            }
            Dp[r - 1][s - 1][j] = p->D[j];
    ...
    //    FILE * fp_s = fopen("smooth.txt","w");
    //    int week;
    //    double weeksec;
    //    for(i=0;i<obs->n;i++){
    //        p = &obs->data[i]; s = p->sat; r = p->rcv;
    //        if (s != 14)continue;
    //        weeksec = time2gpst(p->time,&week);
    //        fprintf(fp_s,"%.3lf %.3lf\n",weeksec,p->P[0]);
    //    }
    //    fclose(fp_s);
}
    ...
    { /* input forward data */
        ...
        else
        {
            ...
            //                if (timediff(obss.data[i].time,obss.data[iobsu].time)>DTTOL) break;
                        /// 这里采用b34g的办法find closest timestamp
        ...
        //TODO:添加相位平滑伪距,多普勒平滑伪距(滑动窗口,定窗口)
        if (popt->smoothmode > 0)
        {
            if (popt->smoothmode == 1)lgccsmooth(&obss, 100, popt);
            else if (popt->smoothmode == 2)lgcdsmooth(&obss, 100, popt);
        }
...
*          double ti        I   processing interval  (s) (0:all) 处理时间间隔。
*          double tu        I   processing unit time (s) (0:all) 处理单元长度。
*          prcopt_t *popt   I   processing options  处理选项，决定“怎么解”。
                                                    比如：
                                                    ...
                                                    模糊度固定策略
                                                        等等
*          solopt_t *sopt   I   solution options  解算选项，决定“怎么输出”。
                                                    比如：
...
*          filopt_t *fopt   I   file options   文件选项，决定“从哪里读”。
                                                    比如：
                                                    ...
                                                    电离层数据文件
                                                        等等
*          char   **infile  I   input files (see below)  输入文件列表，决定“从哪里读”。
                    输入文件可以包含观测数据、导航数据、精密星历/钟差（可选）、sbas日志文件（可选）、
                    ssr消息日志文件（可选）和tec格网文件（可选）。输入文件的类型通过文件扩展名识别。
                    输入文件可以包含通配符（*），如果一个文件包含通配符，则使用通配符展开的多个文件。
                    输入文件可以包含关键字，如果一个文件包含关键字，则关键字被日期、时间、流动站id和基准站id替换，并运行多个会话分析。关键字请参见reppath()。
    ...
    if (ts.time != 0 && te.time != 0 && tu >= 0.0)
    {
        ...
        for (i = 0;;i++)
        {
            ...
            for (j = k = nf = 0;j < n;j++)
            {
                ...
                {
                    /* 对于SP3精密星历或BRDC导航数据，需要包含下一天的数据 */
                    ttte = tte;
                    if (ext && (!strcmp(ext, ".sp3") || !strcmp(ext, ".SP3") ||
                        !strcmp(ext, ".eph") || !strcmp(ext, ".EPH")))
                    {
                        ttte = timeadd(ttte, 3600.0);  /* 精密星历提前1小时 */
                    }
                    else if (strstr(infile[j], "brdc"))
                    {
                        ttte = timeadd(ttte, 7200.0);  /* 广播星历提前2小时 */
                    }
                    /* 进行路径替换（关键字替换）并展开文件 */
