...
const prcopt_t prcopt_default = { /* defaults processing options */
    ...
    30.0,{5.0,100.0,2.0},30.0,              /* maxtdif,maxinno,maxgdop */
...
static int lsq_R_(const double* A, const double* y, const double* R, int n, int m, double* x,
    double* Q)
{
    double* Ay;
    int info;
    ...
    Ay = mat(n, 1);
    AP = mat(n, m);
    //    Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> jac(n,m);
    //    Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> var(m,m);
    //    Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> res(m,1);
    //    memcpy(jac.data(),A,sizeof(double)*n*m);
    //    memcpy(var.data(),P,sizeof (double)*m*m);
    //    memcpy(res.data(),y,sizeof (double)*m*1);
    //
    //    Eigen::MatrixXd result;
    //    result = (jac*var*jac.transpose()).inverse() * (jac * var * res);
    //    std::cout<<"result\n" << result << std::endl;
    //    std::cout<<"jac\n" << jac << std::endl;

    matmul("NN", n, m, m, 1.0, A, P, 0.0, AP);
    matmul("NN", n, 1, m, 1.0, AP, y, 0.0, Ay); /* Ay=A*y */
    matmul("NT", n, n, m, 1.0, AP, A, 0.0, Q);  /* Q=A*A' */
    if (!(info = matinv(Q, n))) matmul("NN", n, 1, n, 1.0, Q, Ay, 0.0, x); /* x=Q^-1*Ay */
    free(P);free(Ay);free(AP);
    return info;
}
extern int lsq_R(const double* A, const double* y, const double* R, int n, int m, double* x,
    double* Q)
{
    double* x_, * xp_, * H_;
    int i, j, k, info, * ix;
    ...
    ix = imat(n, 1); for (i = k = 0;i < n;i++) if (x[i] != 0.0) ix[k++] = i;
    x_ = mat(k, 1); xp_ = mat(k, 1); H_ = mat(k, m);
    /* compress array by removing zero elements to save computation time */
    for (i = 0;i < k;i++)
    {
        x_[i] = x[ix[i]];
        for (j = 0;j < m;j++) H_[i + j * k] = A[ix[i] + j * n];
    }
    /* do kalman filter state update on compressed arrays */
    info = lsq_R_(H_, y, R, k, m, xp_, Q);
    /* copy values from compressed arrays back to full arrays */
    for (i = 0;i < k;i++)
    {
        x[ix[i]] += xp_[i];
    }
    free(ix); free(x_); free(xp_); free(H_);
    return info;
}

extern int lsq_rtk(const double* A, const double* y, const double* R, int n, int m, const double* x, double* xx,
    double* Q, const double* P)
{
    ...
    /* create list of non-zero states */
    ix = imat(n, 1); for (i = k = 0;i < 6;i++) if (x[i] != 0.0 && P[i + i * n] > 0.0) ix[k++] = i;
    x_ = mat(k, 1); xp_ = mat(k, 1); H_ = mat(k, m);
    /* compress array by removing zero elements to save computation time */
    for (i = 0;i < k;i++)
    {
...
extern void matprintT(const char mark[], const double A[], int n, int m, int p, int q)
{
#if MATPRINT
    int i, j;
    printf("\n%s\n", mark);
    for (j = 0;j < m;j++)
    {
        for (i = 0;i < n;i++) printf(" %*.*f", p, q, A[i + j * n]);
        printf("\n");
    }
    printf("\n");
#endif
