...
3. 日志中 `already exists` 为 0；`does not exist` 只允许作为旧模糊度被滑窗主动边缘化后的生命周期提示，不能伴随重复 key 或回滚；
...
最终提交 `0b1a764` 在服务器上使用 `111965–113148` 配置回放；实际数据覆盖到周内秒 `113000`：

- GNSS 因子历元：1021；
- 最大图 key：10349；
- FGO 输出：1035 行，周内秒 111966–113000，无大于 0.01 s 的时间断点；
- 总轨迹输出：10351 行，时间单调，全部数值有限；
- `IndeterminantLinearSystem`、重复变量、优化回滚、边缘化回滚、协方差跳过：均为 0；
- LiDAR/IMU 一致性门限拒绝 30 帧，系统随后继续正常推进；
- `does not exist` 共 238 条，均来自已经边缘化的旧 `N(key)` 生命周期检查，没有伴随重复变量或回滚。

这些结果证明“后端结构与数值稳定性问题”已经闭合，但由于数据目录中没有同时间基准的商业后处理参考轨迹，仍不能据此宣称复现论文的 0.20 m 三维 RMS。
