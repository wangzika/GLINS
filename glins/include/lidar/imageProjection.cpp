    ...
    deskewFlag(0)
{
    ...
    pubExtractedCloud = nh.advertise<sensor_msgs::PointCloud2>("glins/deskew/cloud_deskewed", 1);
    pubLaserCloudInfo = nh.advertise<glins::cloud_info>("glins/deskew/cloud_info", 1);
    pubGroundCloud = nh.advertise<sensor_msgs::PointCloud2>("glins/deskew/cloud_ground", 1);
    allocateMemory();
    resetParameters();
...
void ImageProjection::allocateMemory()
{
    ...
    PatchWorkGroundSeg.reset(new PatchWorkpp<PointXYZIRT>(&nh));
    resetParameters();
}
...
{
    std::lock_guard<std::mutex> lock3(cloudLock);
...
void ImageProjection::cloudExtraction()
{
    ...
    for (int i = 0; i < N_SCAN; ++i)
    {
        ...
        {
            if (rangeMat.at<float>(i, j) != FLT_MAX && groundMat.at<int8_t>(i, j) != 1)
            {
                // mark the points' column index for marking occlusion later
                cloudInfo.pointColInd[count] = j;
                // save range info
                cloudInfo.pointRange[count] = rangeMat.at<float>(i, j);
                if (fullCloud->points[j + i * Horizon_SCAN].z < -2) continue;
